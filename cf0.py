...
class System(cf_spec.System): 
    ...
    aggregation_methods = ['mean', 'median', 'log', ] # other options: 'enet', 'knn', 'qda', ... 
...
def run_cluster_analysis(F, U=None, X=None, kind='user', n_clusters=-1, index=0, params={}, **kargs):
    ...
    # parameters 
    tSaveData = kargs.get('save', False)
    ...
    if tSavePlot: 
        ...
        if not output_path: 
            ...
            if params: 
                ...
                fname = '{prefix}.M-{dataset}-{suffix}-{index}.{ext}'.format(prefix='similarity', dataset=dset_id, suffix=file_type, index=index, ext=ext)
            else: 
                fname = '{prefix}.M-{suffix}-{index}.{ext}'.format(prefix='similarity', suffix=file_type, index=index, ext=ext)
...
def reconstruct(C, X, P, Q, policy_opt='rating', policy_replace='rating', 
        replace_subset=True, replace_all=False, params={}, null_marker=0, name='X', verify=False, binarize=False):
    """
    ...
    if policy_opt.startswith('pref'):
        ...
        Xh = Mask = uc.compute_preference(P, Q, canonicalize=True, binarize=binarize, verify=True) # use Tpf as a mask
        ...
        div("(reconstruct) Reconstructed matrix holds preference scores (binarized? {tval})".format(tval=binarize), symbol='#')
        ...
        if replace_all: # params['predict_probs']: 
            ...
            div("> reconstructing the entire proba table ...", symbol='#')
            ...
            print("... replacement mode: replacing bad 'ratings' | (n_zeros: {0} =?= n_masked: {1}, ratio: {2}) | dim(X): {3}".format(
                n_zeros, n_masked, n_zeros/(Mask.shape[0]*Mask.shape[1]+0.0), X.shape))

            # wherever Cui was zerioed out (low confidence), replace these entries with new estimates
            Xh = uc.replace(P, Q, X=(Mask, X), canonicalize=True, 
                    fill=null_marker, predict_func=ua.predict_by_factors, name=name)
...
def wmf_ensemble_iter(data, params, hyperparams={}, indices=[], vars=['wmfCV', 'wmfMetrics', ], kind='als', fold=-1, outer_fold=-1, null_marker=0, verbose=1, 
    ...
    div("(wmf_ensemble_iter) Completed C(R) | Cycle {cycle} |  dim(Cui): {dim}, filter_axis: {fdim} | conf_measure: {measure}, optimization: {opt} | predict ALL probabilities? {tval} | policy_threshold: {p_th}".format(cycle=fold, 
    ...
    Rh = reconstruct(Cr, R, Pr, Qr, policy_opt=params['policy_opt'], policy_replace=params['policy_replace'], 
            replace_subset=params['replace_subset'], params=params, null_marker=null_marker, name='R')
    ...
    Ct, Ct_bar = uc.evalConfidenceMatrix(X=T, L=[], U=U, 
            M=(R, L_train, Cr),  # set M/message to R, L so that we can use proba thresholds from R to estimte labels in T; optionally, use Cr to identify useless classifiers (producing no correct positive predictions)
            ratio_users=params['ratio_users'], 
    ...
    Th = reconstruct(Ct, T, Pt, Qt, policy_opt=params['policy_opt'], policy_replace=params['policy_replace'], 
            replace_subset=params['replace_subset'], params=params, null_marker=null_marker, name='T')
    ...
    if post_hoc_analysis:   # only triggered after model selection cycle is complete and 'best params' has been determined
        ...
        for file_type in file_types: 
            ...
            for i, aggr in enumerate(aggregation_methods):    # defined earlier e.g. ['mean', 'median']
                ...
                if aggr in ['mean', 'median', ]:   # System.simple_aggretation
                    if isPreferenceScore: 
                        if file_type.startswith('pri'):
                            pv = uc.combiner(T, aggregate_func=aggr)
                        else: 
                            pv = uc.predict_by_preference(T, Th, canonicalize=True, binarize=False, name='Th', aggregate_func=aggr, fallback_to_mean=False, min_pref=0.1) # verify/False
                    else: 
                        pv = uc.combiner(X_test, aggregate_func=aggr)  # T vs Th
                else:
                    ...
                    if isPreferenceScore: 
                        ...
                        if file_type.startswith('post'):
                            Rb = uc.canonicalize_pref(Rh, name='Rb', binarize=True, verify=2)
                            Tb = uc.canonicalize_pref(Th, name='Tb', binarize=True, verify=2)
                            X_train = R * Rb
                            X_test = T * Tb
        ...
        if enable_cluster_analysis: 
            ...
            for fdim in filter_axes: 
                ...
                run_cluster_analysis(Pr, U=U, X=R, kind='user', n_clusters=n_classifiers, index=outer_fold, params=params,
                ...
                # test split
                run_cluster_analysis(Pt, U=U, X=T, kind='user', n_clusters=n_classifiers, index=outer_fold, params=params,
...
def model_select_core(data, params, param_grid, vars=['wmfCV', 'wmfMetrics', ], kind='als', dev_ratio=None, max_dev=None, n_trial=0, outer_fold=-1, null_marker=0, 
        post_hoc_analysis=False, enable_cluster_analysis=False, verbose=True): 
    """
    ...
    ########################################################################################
    inner_fold = n_trial
    print('(model_select_core) MS inner cycle #%d | param_grid: %s > trigger model selection? %s  ... (verify) #' % (inner_fold, param_grid, tModelSelection))
    ...
    if not tModelSelection: 
        ...
        div(message='(model_select_core) size(train_dev): {n} | to sample only a subset? {tval} | n_total(train_dev): {N}'.format(n=max_dev if max_dev is not None else df_train_dev.shape[0], 
        ...
        models = Parallel(n_jobs = -1, verbose = 1)(delayed(wmf_ensemble_iter)(
                            data=uc.shuffle_split(df_train_dev, ratio=dev_ratio, max_size=max_dev, index=inner_fold), # uc.shuffle_split return a 5-tuple: (R, Td, L_train, L_dev, U)
...
def wmf_ensemble_model_select(params, param_grid={}, vars=['wmfCV', 'wmfMetrics', ], kind='als', n_trials=1, 
        dev_ratio=0.2, test_ratio=0.3, max_dev=None, null_marker=0, fold=-1, unbag=False, 
    ...
    train_df, train_labels, dev_df, dev_labels, test_df, test_labels = common.shuffle_split_cv(input_path, split_number=3, dev_ratio=dev_ratio, max_size=None, fold=outer_fold, verbose=True)
    ...
    if tMode.startswith('para'): 
        ...
        best_models = Parallel(n_jobs = -1, verbose = 1)(delayed(model_select_core)(
                                                            data=D_minus, params=params, param_grid=param_grid, vars=vars, dev_ratio=dev_ratio, max_dev=max_dev, kind=kind, null_marker=null_marker, 
    ...
    finalModel = wmf_ensemble_iter(data=D, params=params, vars=vars, indices=Ix, kind=kind, null_marker=null_marker, 
...
def wmf_ensemble_fold(fold, params, hyperparams={}, indices=[], vars=['wmfCV', 'wmfMetrics', ], kind='als', null_marker=0, verbose=1, 
    ...
    div("... Completed conficence matrix for training data C(R) | Cycle {0} | n_factors: {1}, alpha: {2} | dim(Cui): {3} | conf: {4} optimization: {5} | replace bad probabilities only? {6}".format(fold, 
        params['n_factors'], params['alpha'], str(Cr.shape), params['policy'], params['policy_opt'], params['replace_subset']), symbol='#', border=1)
    ...
    div("... Completed conficence matrix for test data C(T) | Cycle {0} | n_factors: {1}, alpha: {2} | dim(Ct): {3} | conf: {4} optimization: {5} | replace bad probabilities only? {6}".format(fold, 
        params['n_factors'], params['alpha'], str(Ct.shape), params['policy'], params['policy_opt'], params['replace_subset']), symbol='#', border=1)
    ...
    div("... Completed rating matrix reconstruction > FOLD: {0} | preference scores? {1}, action='{2}'".format(
        fold, isPreferenceScore, 'Replace Subset' if params['replace_subset'] else 'Replace All')) # predict => predict probabilities
    ...
    # header = MFEnsemble.header_model_evaluation # ['method', 'score', 'posterior_score', 'mean_score', ] ... used in evaluate.analyzePerf2()
    perf, pv = analyzePerf(L_test, Th, method=method_id, aggregate_func=aggregate_func, T=T, fold=fold, mode='mean')
    wmfMetrics.append( perf ) # optional params: T, fold (used to comparing the utility between T and Th)  # Th: final prediction
...
def test_combiner_subsampling(context, dset_id, evaluation=True, **kargs):
    ...
    aggregation_methods = ['mean', 'median',] + ['latent_mean', 'latent_mean_masked', ]  # stackers: ['log', ] 
    for dtype in ['prior', 'posterior', ]: 
        # 1. match
...
def test_wmf_probs_suite(**kargs): 
    ...
    System.descriptions = descriptions = \
            {0: 'baseline', 
                ...
                1: 'item_centered', 2: 'user-centered', 
                    ...
                    5: 'item-centered-low-support', 6: 'user-centered-preference', 
    ...
    elif setting in (6, 'user-centered-preference'): 
        kargs['policy'] = 'user'
        kargs['policy_test'] = 'user' # policy_filter_test   # filtering along the axis of item
