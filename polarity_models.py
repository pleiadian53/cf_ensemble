...
# Utilities with hooks to a broader class of predictive models (e.g. cf_models)
...
def make_seq2seq_training_data(R, Po=None, L=None, include_label=True, **kargs): 
    ...
    X and Y should be in 3D, which is required by TF's LSTM 
    """
    # import pmodel_pipeline
    return pmodel_pipeline.make_seq2seq_training_data(R, Po, L, include_label=include_label, **kargs)

def predict_filter(model_seq, X, batch_size): 
    return pmodel_pipeline.predict_filter(model_seq, X, batch_size)

def make_seq2seq_training_data2(R, Po=None, L=None, include_label=True, **kargs): 
    """
    Create mask prediction training data set where the true labels (Y) consists of the following two feature dimensions: 
    ...
    X and Y should be in 3D, which is required by TF's LSTM 
    """
    return pmodel_pipeline.make_seq2seq_training_data2(R, Po, L, include_label=include_label, **kargs)

# [todo]
def predict_filter2(model_seq, X, batch_size, **kargs): 
    return pmodel_pipeline.predict_filter2(model_seq, X, batch_size, **kargs)
