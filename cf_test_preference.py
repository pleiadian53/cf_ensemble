...
def run_pref_stacker_suite(**kargs): 
    ...
    params['mask'] = kargs.get('mask', True) # mask FP and FN? this makes Cui 'sparse'
    ...
    for dataset in datasets: 
        ...
        for method in ['mean', ]:
            ...
            for fold in range(n_fold): 
                ...
                if method == 'mean': 
                    T_pref = test_pref.values.T   # transpose to get users-by-items format
                    T = test_df.values.T

                    predictions = combiner(T_pref, aggregate_func='pref', T=T)

                    perf = analyzePerf(test_labels, predictions, method='{combiner}_pref'.format(combiner_type=method))
...
def run_pref_stacker(**kargs): 
    ...
    for dataset in [dataset, ]: 
        ...
        ### end foreach method 
        div('(run_pref_stacker) Completed method {0} #'.format(method))
    ### end foreach dataset 
...
def wmf_ensemble(**kargs):
    def save_meta_tset(params): # module: tset
        meta = '{confidence_type}_{policy}'.format(confidence_type=params['mode'], policy=params['policy'])
        if not params['mask']: meta = '{meta}_dense'.format(meta=meta)
        tset_id = MFEnsemble.name_tset(method='wmf', n_factors=params['n_factors'], alpha=params['alpha'], meta=meta) # meta: extra info needed to differentiate training data
    ...
    for fold in range(n_fold): 
        ...
        if not params['policy'].startswith('t'):
            Cui, R, T, L_train, L_test, U = uc.toConfidenceMatrix(fold, p_threshold=p_th, 
                fill=missing_value, verbose=True, is_augmented=True, 
                mode=params['mode'], alpha=params['alpha'], mask=params['mask'])
            Cui_bar = None
        else: 
            # Cui, Cui_bar may not be sparse
            Cui, Cui_bar, R, T, L_train, L_test, U = uc.toConfidenceMatrix2(fold, p_threshold=p_th, 
                fill=missing_value, verbose=True, is_augmented=True, 
                mode=params['mode'], alpha=params['alpha'])  # is_augmented => consider the confidence level in the test split
        ...
        for kind in kinds:
            if kargs.get('save', True): save_meta_tset(params=params)
...
def test_stacker(**kargs):
    ...
    if exact_match: 
        for method in kargs.get('methods', ['wmf', 'nmf', ]): 
            run_stacker_suite(context='{mf_method}_stackers'.format(mf_method=method), method=method, exact=True, 
                run_bp_stacker=True, 
                    topk_bps=3, topk_bp_stackers=3,  # only comparing with the top 3 baseline methods 
                    parallelize=True, reconstructed_testset=True, pref_aug=kargs.get('pref_aug', True))  # pref_model: True, then don't do stacking on top of preference-augmented dataset
...
def test_wmf_probs(**kargs): 
    ...
    ### model selection parameteres

    ## a. fix (nf, alpha), vary mode
    #     {'n_factors':[10, ], 'alpha':[100, ], 'mode': ['brier', 'ratio']}
    ## b. fix mode, vary (nf, alpha)

    meta_params = {'mode': 'brier', 'policy': 'preference', 'mask': True, 'pref_aug': True}  # options: {'rating', 'preference', 'label', 'tradeoff'}
    param_grid = kargs.get('param_grid', {'n_factors':[10, ], 'alpha':[100, ]})

    # file ID
    meta = '{confidence_type}_{policy}'.format(confidence_type=meta_params['mode'], policy=meta_params['policy'])
    if not meta_params['mask']: meta = '{meta}_dense'.format(meta=meta)
    ...
    for params in list(ParameterGrid(param_grid)):  # a list of dictionaries containing target (hyper)parameters
        perf = wmf_ensemble_suite(run_bp=False, run_bp_stacker=False,  
                    run_wmf_ensemble=True, run_wmf_stacker=False, run_wmf_similarity=False, 
                        ...
                        mode=meta_params['mode'], policy=meta_params['policy'], mask=meta_params['mask'])
    ...
    post_analysis(perfAll, context='test_wmf_probs', highlight=['wmf', ], metrics=['fmax', 'auc', ])

    # now test the quality using stacking methodology 
    for params in list(ParameterGrid(param_grid)): 
        tset_id = MFEnsemble.name_tset(method='wmf', n_factors=params['n_factors'], alpha=params['alpha'], meta=meta) # meta: extra info needed to differentiate training data
        test_stacker(context='test_wmf_probs_via_stackers-{0}'.format(tset_id), methods=[tset_id, ], exact=True, pref_aug=meta_params['pref_aug'])
