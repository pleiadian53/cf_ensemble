...
def run_stacker_suite(base_perf=None, **kargs):
    ...
    if base_perf is not None: 
        ...
        post_analysis(perfMetrics, context='stacker_suite', highlight=['wmf', 'nmf', ])
...
def wmf_ensemble_suite(base_perf=None, **kargs):
    ...
    if kargs.get('run_wmf_stacker', True): 
...
def wmf_ensemble_suite_multimodel(**kargs):
    ...
    if kargs.get('plot', True): 
        ...
        for metric in ['fmax', 'auc', 'fmax_negative', ]: 
            ...
            sorted_pairs = perfAll.sort(metric=metric, reverse=greater_is_better, verbose=False)
            
            # target_set: if specified, methods that match the keywords will be highlighted
            s = evaluate.format_ranked_list(sorted_pairs, metric=metric, topk=None, verbose=False, target_set=['wmf',  ])
            print(s.rjust(len(s)+indent_level, ' '))

            # plot 
            # MFEnsemble.name_performance_plot(method=category, metric=metric, size=size, aspect='comparison', domain=domain)
            file_name = '{method}_{metric}_comparison-N{size}-D{domain}'.format(method=category, 
                metric=metric, size=perfAll.n_methods(), domain=Domain)
            evaluate.plot_performance(perfAll, metric=metric, ascending=True, domain=Domain, file_name=file_name)
...
def wmf_ensemble_modelselect(**kargs):
    ...
    # 'performance_metrics-{kind}-{domain}'.format(kind='model-select', domain=Domain)
    perfAll.save(file_name=perfAll.my_shortname(context='model-selection', domain=Domain))  # only saves the table for now ... 02.05.19

    # total ranking
    indent_level = 2
    greater_is_better = True
    category = 'wmf'  # inspect.stack()[0][3].split('_')[0]
    for metric in ['fmax', 'auc', 'fmax_negative', ]: 
        ...
        file_name = '{method}_{metric}_comparison-N{size}-D{domain}'.format(method=category, 
                metric=metric, size=perfAll.n_methods(), domain=Domain)
        evaluate.plot_performance(perAll, metric=metric, ascending=True, domain=Domain, file_name=file_name)
...
def suite(**kargs):
    ...
    ### Weighted MF via ALS
    # perf_wmf = wmf_ensemble_suite(base_perf)
...
def test_stacker(**kargs):
    ...
    for kind in ['user', 'item']: # separate user models and item models
        run_stacker_suite(context='wmf_{kind}_stackers'.format(kind=kind), run_bp_stacker=True, parallelize=True, method='wmf', keywords=[kind, ]) 
        run_stacker_suite(context='nmf_{kind}_stackers'.format(kind=kind), run_bp_stacker=True, parallelize=True, method='nmf', keywords=[kind, ]) 
...
def test_nmf_vs_wmf(**kargs):
    ...
    for nf in [10,  ]: 
        perfWMF = wmf_ensemble_suite(base_perf=None, n_factors=nf, save=True, 
                    run_stacker=False, run_wmf_ensemble=False, run_wmf_stacker=False, run_wmf_similarity=True, 
                        run_clustering=True) # set save to True to save R' and T' (reproduced probabilities)
    ...
    perfAll = post_analysis(perfMetrics, context='wmf_vs_nmf')
