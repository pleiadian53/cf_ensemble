...
def run_cluster_analysis(X, F, kind='user', save=False):
    """
    ...
    if kind.startswith( ('u', 'cl', )):   # user, classifier 
        assert R.shape[0] == factors.shape[0]
    elif kind.startswith( ('i', 'd',) ):  # item, data
        assert R.shape[1] == factors.shape[1]
    print('(clustering) Running cluster analysis on {role} represented by {n} factors'.format(role=kind, n=F.shape[1]))

    S = uc.evalSimilarityByLatentFeatures(F, epsilon=1e-9)
    if save: save_array(S, kind) # fold
...
def wmf_ensemble_iter(data, params, hyperparams={}, indices=[], vars=['wmfCV', 'wmfMetrics', ], kind='als', fold=-1, null_marker=0, verbose=1, 
    ...
    div("... Completed conficence matrix for training data C(R) | Cycle {0} | n_factors: {1}, alpha: {2} | dim(Cui): {3} | conf: {4}, conf_measure: {5}, optimization: {6} | predict ALL probabilities? {7} | policy_threshold: {8}".format(fold, 
...
def wmf_ensemble_model_select(params, param_grid={}, vars=['wmfCV', 'wmfMetrics', ], kind='als', n_trials=1, 
        dev_ratio=0.2, test_ratio=0.5, max_dev=None, policy_ms='freq', null_marker=0, fold=-1, unbag=False, save=True, verbose=True, input_path=None):
    """
    ...
    div(message='(wmf_ensemble_model_select) Cycle {index} | Best of the best params across n_trials={nt} > n_factors: {nf}, alpha: {a} | policy_ms={pms}, score: {score}... (verify) #'.format(index=fold, nt=n_trials, 
...
def wmf_ensemble(**kargs):
    ...
    if n_runs > 1 or n_runs_modelselect > 1: 
        ...
        if tModelSelection:  # True only if param_grid has effectively multiple parameters to choose from
            ...
            #################
            div("(wmf_ensemble) Found {n} sets of 'best params' with {ns} scores ... (verify) #".format(n=M.size(), ns=M.size_bags()))
            ...
            print( us.format_sort_dict(dict(freq_models), reverse=True, padding=5, title="(result) Sorted 'best parameters' by frequency | metric={metric} ...".format(metric='fmax')) ) # symbol='#', border=1

            mean_models = M.aggregate()
            print( us.format_sort_dict(mean_models, reverse=True, padding=5, title="(result) Average performance of 'best parameters' | metric={metric} ...".format(metric='fmax')) ) # symbol='#', border=1
            ...
            if policy_ms.startswith('freq'):  # find the overall popularity of the hyperparams across multiple runs
                modelCount = {}
                for model in vmap['rank']:  # best_params, best_score = best_models[0], best_models[1]  # best_params: a frozen dictionary
                    hyparams, count = model
                    print("(debug) hyperparams: {p} => count: {c}".format(p=hyparams, c=count)) 
                ...
                div('(result) Globally best params (n_runs={nruns}): {params} | frequency: {freq}'.format(nruns=n_runs, params=best_params_nruns, freq=best_score_nruns), symbol='#', border=2)
        ...
        ret['best_params'] = best_params = dict(M.sort(by=policy_ms)[0][0])  # sort by aggregate function defined in Metrics constructor
